...
%%%
%%% @end
%%% Created : 16. 2月 2023 21:25
%%%-------------------------------------------------------------------
-module(websocket_decode).
...
%%                      io:format("FIN:~w, RSV1:~w, RSV2:~w, RSV3:~w, Opcode:~w, Mask:~w, Len:~w ~n", [FIN, RSV1, RSV2, RSV3, Opcode, Mask, Len]),
                      {PayloadLen, Data} = if
                                               Len < 126 ->
                                                   {Len, PayloadData};
                                               Len == 126 ->
...
mask(Data) ->
    MaskKey = erlang:system_time(second),
